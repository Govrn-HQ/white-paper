...
This becomes a big problem when it has to do with governance and democracy. Rather than a democracy, it enables a Technocracy.
...
Aaron/Christine convo:
- Current problems of experts co-opting narratives as they only have access to the tools
- If we build tools that only technologists know how to use, then it doesn’t make things more equitable
- Building more accesible tools that no one can use is just as bad as the status quo
- Ope source tools transition the problem from "accessibility" to "learning how to use the tools"
- It's less about the technology and more about the processes and pathways that are opened up
